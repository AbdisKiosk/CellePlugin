...
public class PermittedCellCompletion implements NamedCompletionHandler {
    ...
    public List<String> complete(@NonNull CompletionMeta completionData, @NonNull ArgumentMeta argument, @NonNull Invocation invocation, @NonNull CommandData data) {
        ...
        return stores.getCellStore().getPermittedCells(stores.getUserStore().get((Player) sender)).stream()
                .map(cell -> cell.getName().toLowerCase())
